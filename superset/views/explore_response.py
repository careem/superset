...
class ExploreResponse():
    def __init__(self, form_data: Dict[str, Any], slc: Slice, query_context: Any):
        self.slc = slc
    ...
    def __getBaseDatasources(datasourceIds: List[int], datasourceTypes: List[str]) -> List[BaseDatasource]:
        ...
        for index, value in enumerate(datasourceIds):
            newDatasource = ConnectorRegistry.get_datasource(
                cast(str, datasourceTypes[index]), value, db.session
            )
